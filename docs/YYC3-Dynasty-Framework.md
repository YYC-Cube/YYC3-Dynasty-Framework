---
file: YYC3-Dynasty-Framework.md
description: YYC³ Dynasty 使用部署分析 — 安装、配置、启动与运维全流程指南
author: YanYuCloudCube Team <admin@0379.email>
version: v1.0.0
created: 2026-03-15
updated: 2026-06-08
status: stable
tags: [deployment],[guide],[architecture]
category: technical
language: zh-CN
...
三省六部 · Dynasty Framework 是一个基于 OpenClaw 的 AI 多 Agent 协作系统，采用中国古代三省六部制度设计，包含 12 个 AI Agent（11 个业务角色 + 1 个兼容角色）。系统比传统 Multi-Agent 框架多一层制度性审核和实时看板功能，实现了真正的分权制衡。
...
```bash
# 安装 systemd 服务
sudo cp Dynasty Framework.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable edict
sudo systemctl start edict

# 或使用管理脚本
bash Dynasty Framework.sh start    # 启动
bash Dynasty Framework.sh status   # 查看状态
bash Dynasty Framework.sh restart  # 重启
bash Dynasty Framework.sh stop     # 停止
```
...
Dynasty Framework · 三省六部 项目采用中国古代三省六部制度设计 AI 多 Agent 协作架构，通过制度化的审核机制和实时看板，实现了比传统 Multi-Agent 框架更可靠、更可观测的协作系统。
