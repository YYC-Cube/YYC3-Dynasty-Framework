#!/usr/bin/env python3
"""
file: kanban_update.py
description: 看板任务更新工具 · 状态机校验 + 旨意数据清洗 + 标题校验
author: YanYuCloudCube Team
version: v1.0.0
created: 2026-02-26
updated: 2026-06-08
status: active
tags: [kanban],[state-machine],[data-cleaning]

brief: 供各省部 Agent 调用的看板任务更新 CLI

details:
- 操作 data/tasks_source.json（JSON 看板模式）
- 内置 _VALID_TRANSITIONS 状态机校验，非法跳转被拒绝
- 旨意标题/备注自动清洗（路径、元数据、无效前缀剥离）
- 如已部署 edict/backend（Postgres + Redis），请使用 API 端点代替

dependencies: file_lock, utils
exports: cmd_create, cmd_move, cmd_done, cmd_block, cmd_unblock
notes: 两种模式（JSON / Postgres）互相独立，数据不会自动同步
...
def cmd_state(task_id, new_state, now_text=None):
    ...
    if rejected[0]:
        ...
        _append_audit(
            ...
            old_state[0],
            new_state,
            f"需 {CONFIRM_AUTHORITY.get(old_state[0], 'shangshu')} 确认",
        )
    else:
