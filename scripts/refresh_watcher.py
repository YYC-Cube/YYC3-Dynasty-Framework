#!/usr/bin/env python3
"""
file: refresh_watcher.py
description: 刷新监控守护进程 · debounce 合并多 Agent 并发刷新请求
author: YanYuCloudCube Team
version: v1.0.0
created: 2026-03-01
updated: 2026-06-08
status: active
tags: [daemon],[watcher],[debounce]

brief: 常驻进程，监控信号文件，debounce 后执行 refresh_live_data.py

details:
- 替代 kanban_update.py 中每次操作都 fork 子进程的方式
- 多 Agent 并发时，200 次 touch → 合并为 1 次 refresh
- 部署方式: systemd (edict.service) 或 docker-compose

dependencies: watchdog or os.watch
"""
...
_BASE = (
    pathlib.Path(os.environ.get("EDICT_HOME", "")).resolve()
    if os.environ.get("EDICT_HOME")
