...
description: Skill 管理工具 · 添加/更新/查看/移除本地与远程 Skills
author: YanYuCloudCube Team
version: v1.0.0
created: 2026-03-01
updated: 2026-06-08
status: active
tags: [util],[skill],[agent]

brief: 管理各省部 Agent 的 Skill 配置

details:
- 支持从本地或远程 URL 添加、更新、查看和移除 skills
- 支持 add-remote / list-remote / update-remote / remove-remote 命令

dependencies: argparse, json, pathlib, requests
notes: 远程 URL 需为可公开访问的 SKILL.md 文件
"""
...
from utils import get_openclaw_home, now_iso, safe_name
    ...
    for attempt in range(1, retries + 1):
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "OpenClaw-SkillManager/1.0"}
            )
...
def add_remote(
    ...
    if not safe_name(agent_id) or not safe_name(name):
        print("❌ 错误：agent_id 或 skill 名称含非法字符")
        return False
...
def update_remote(agent_id: str, name: str) -> bool:
    ...
    try:
        ...
        # 重新下载
        return add_remote(
            agent_id, name, source_url, source_info.get("description", "")
        )
    except Exception as e:
        print(f"❌ 更新失败：{e}")
        return False
...
def import_official_hub(agent_ids: list) -> bool:
    ...
    for skill_name, url in OFFICIAL_SKILLS_HUB.items():
        ...
        for agent_id in target_agents:
            total += 1
            ok = add_remote(
                agent_id, skill_name, effective_url, f"官方 skill：{skill_name}"
            )
            if not ok and effective_url == url:
                ...
                for fb_base in _FALLBACK_HUB_BASES:
                    ...
                    print(f"   🔄 尝试镜像: {fb_url}")
                    ok = add_remote(
                        agent_id, skill_name, fb_url, f"官方 skill：{skill_name}"
                    )
                    if ok:
    ...
    if failed:
        ...
        print("\n💡 排查建议:")
        print(
            f"   1. 检查网络: curl -I {OFFICIAL_SKILLS_HUB_BASE}/code_review/SKILL.md"
        )
        print("   2. 设置代理: export https_proxy=http://your-proxy:port")
        print(
            f"   3. 使用镜像: export {_HUB_BASE_ENV}=https://ghproxy.com/{OFFICIAL_SKILLS_HUB_BASE}"
        )
        print(
            f'   4. 自定义源: echo "https://your-mirror/skills" > {OCLAW_HOME / "skills-hub-url"}'
        )
        print(
            "   5. 单独重试: python3 scripts/skill_manager.py add-remote --agent <agent> --name <skill> --source <url>"
        )
...
def main():
    ...
    import_parser = subparsers.add_parser(
        "import-official-hub", help="从官方库导入 skills"
    )
    import_parser.add_argument(
        "--agents", default="", help="逗号分隔的 Agent IDs（可选）"
    )

    # check-updates
    check_parser = subparsers.add_parser("check-updates", help="检查更新（未来功能）")
    check_parser.add_argument(
        "--interval", default="weekly", help="检查间隔 (weekly/daily/monthly)"
    )
